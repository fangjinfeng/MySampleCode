//
//  FJFGradualCurveGraphBackgroundView.m
...
@implementation FJFGradualCurveGraphBackgroundControlsProperty
...
@implementation FJFGradualCurveGraphBackgroundViewStyle
#pragma mark - Life Circle
- (instancetype)init {
    if (self = [super init]) {
        _lineViewHeight = 0.5f;
        _leftEdgeSpacing = 20.0f;
        _rightEdgeSpacing = 10.0f;
        _verticalLabelWidth = 26.0f;
        _verticalTextArray = @[@"40", @"20", @"0", @"-20", @"-40"];
        _horizontalTextArray = @[@"0", @"20", @"40", @"60", @"80", @"100"];
        _verticalViewSpacing = 40.0f;
        _labelToVerticalLineSpacing = 4.0f;
        _curveGraphViewTopEdgeSpacing = 15.0f;
        _hideVerticalFirstLabel = NO;
        _hideVerticalLineView = YES;
        _horizontalLabelToCurveGraphViewSpacing = 6.0f;
        _curveGraphBackgroundViewWidth = [UIScreen mainScreen].bounds.size.width - 2 * 12.0f;
        _controlsPropertyStyle = [[FJFGradualCurveGraphBackgroundControlsProperty alloc] init];
    }
    return self;
}
@end

@interface FJFGradualCurveGraphBackgroundView()
...
// viewStyle
@property (nonatomic, strong) FJFGradualCurveGraphBackgroundViewStyle *viewStyle;
@end

@implementation FJFGradualCurveGraphBackgroundView

#pragma mark - Life Circle
- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame viewStyle:[[FJFGradualCurveGraphBackgroundViewStyle alloc] init]];
}
...
- (void)updateCurveGraphLineContainerView {
    ...
    CGFloat leftEdgeSpacing = _viewStyle.leftEdgeSpacing;
    ...
    self.verticalLineView.hidden = _viewStyle.hideVerticalLineView;
}
...
- (CGRect)verticalLabelFrameWithLabelIndex:(NSInteger)labelIndex {
    CGFloat labelX = CGRectGetMaxX(self.curveGraphLineContainerView.frame) + _viewStyle.labelToVerticalLineSpacing;
    CGFloat labelWidth = _viewStyle.verticalLabelWidth;
    CGFloat labelHeight = [FJFGradualCurveGraphView sizeForFont:_viewStyle.controlsPropertyStyle.verticalFont contentString:_viewStyle.verticalTextArray[labelIndex]].height;
...
- (CGRect)horizontalLabelFrameWithLabelIndex:(NSInteger)labelIndex {
    ...
    CGFloat labelHeight = [FJFGradualCurveGraphView sizeForFont:_viewStyle.controlsPropertyStyle.horizontalFont contentString:_viewStyle.horizontalTextArray[labelIndex]].height;
...
- (CGFloat)curveGraphContainerViewWidth {
    CGFloat leftEdgeSpacing = _viewStyle.leftEdgeSpacing;
    CGFloat rightEdgeSpacing = _viewStyle.rightEdgeSpacing + _viewStyle.verticalLabelWidth + _viewStyle.labelToVerticalLineSpacing;
